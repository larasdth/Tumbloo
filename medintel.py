...
import requests
from fpdf import FPDF # Menggunakan fpdf2
...
with st.sidebar:
    ...
    -   **Q: Mengapa grafik tidak muncul setelah memproses data?**
        -   A: Pastikan semua kolom yang diperlukan diisi dengan benar (untuk input manual) atau format CSV Anda benar (untuk unggahan CSV). Periksa konsol Streamlit untuk kesalahan JavaScript.
...
if not st.session_state.cleaned_data.empty:
    ...
    is_dark_mode = (current_theme == "dark") or (st.get_option("theme.base") == "streamlit") # Asumsi 'streamlit' theme juga gelap
        ...
        try:
            with st.spinner("Menghasilkan wawasan dengan Gemini Flash..."):
                ...
                if not df['sentiment'].empty and len(df['sentiment'].value_counts()) >= 1:
                    all_chart_insights.append(f"Sentiment analysis shows {df['sentiment'].value_counts().index[0]} is dominant.")
                if not df.empty and len(df.groupby('platform')['engagements'].sum()) >= 1:
                    all_chart_insights.append(f"Platform {df.groupby('platform')['engagements'].sum().idxmax()} has highest engagements.")
                if not df['media_type'].empty and len(df['media_type'].value_counts()) >= 1:
                    all_chart_insights.append(f"Media type {df['media_type'].value_counts().index[0]} is most frequent.")
                if not df['location'].empty and len(df['location'].value_counts()) >= 1:
                    all_chart_insights.append(f"Location {df['location'].value_counts().index[0]} is most active.")
...
if not st.session_state.cleaned_data.empty:
    ...
    if st.button("Unduh Laporan sebagai PDF", disabled=download_pdf_disabled, key="download_pdf_button_streamlit"):
        ...
        if not platform_engagements_pdf.empty:
            all_chart_insights_for_pdf.append(f"- '{platform_engagements_pdf['platform'].iloc[0]}' adalah platform dengan kinerja terbaik.")
